...
Everything below is served by the demo proxy. Install it once:
...
### A. Browse the API documentation

The OpenAPI 3.1 spec ([`openapi.yaml`](./openapi.yaml)) is rendered as an
interactive **Scalar** reference site.

```bash
pnpm demo                    # starts the server (mock mode is fine for docs)
```
...
| URL | Page |
| --- | --- |
| <http://localhost:5175/> | **Landing page** — presents the MCP + API with links to everything |
| <http://localhost:5175/api.html> | **Scalar** API reference (interactive, from `openapi.yaml`) |
| <http://localhost:5175/docs.html> | This guide, rendered as a styled page |
| <http://localhost:5175/demo.html> | The live React demo app |
| <http://localhost:5175/openapi.yaml> | The raw OpenAPI document |

The Scalar page needs no build step — it loads the spec from `/openapi.yaml` at
runtime. You can also paste `openapi.yaml` into any OpenAPI tool (Swagger UI,
Postman, `openapi-generator`).

### B. Use the MCP server (with Claude / any MCP client)

The [`mcp/`](./mcp/) server exposes the API as tools + resources.

```bash
cd mcp
pnpm install
pnpm demo                    # TR_DEMO=1 — mock data, no credentials/browser
node smoke-test.mjs          # optional: verify tools/resources over stdio
```

Register it with Claude Code:

```bash
# mock mode (no credentials): pass the env flag before the `--` separator
claude mcp add trade-republic --env TR_DEMO=1 -- node /ABSOLUTE/PATH/mcp/server.mjs

# real data: same command without --env TR_DEMO=1 (install demo deps + playwright first)
claude mcp add trade-republic -- node /ABSOLUTE/PATH/mcp/server.mjs
```
...
**Instructions for the model / user** — the tool flow is stateful:

1. `tr_login` `{ phoneNumber, pin }` → solves WAF, triggers 2FA, returns a `processId`.
2. Read the 4-digit code from your Trade Republic app / SMS.
3. `tr_verify` `{ processId, code }` → completes login and opens the WebSocket.
4. Then query freely: `tr_status`, `tr_positions`, `tr_cash`, `tr_timeline`,
   `tr_portfolio_chart { range }`.

Resources `tr://openapi` and `tr://guide` let the model read the spec and this
guide directly. Tools are **read-only** — no order/payout tools are exposed.
Full details in [`mcp/README.md`](./mcp/README.md).

### C. Run the demo app with REAL data (no mock)

```bash
cd demo
pnpm install
npx playwright install chromium      # one-time: needed to solve the WAF challenge
pnpm start                           # REAL mode (no TR_DEMO)
open http://localhost:5175
```

1. Enter your **phone number + PIN**. The server launches headless Chromium,
   harvests the `aws-waf-token`, then calls `POST /api/v1/auth/web/login`.
2. Enter the **4-digit 2FA code** from your TR app / SMS.
3. The chart + live WebSocket panels (positions / cash / timeline) load, and the
   proxy keeps the 300 s session refreshed automatically.

> **WAF error on login?** The challenge wants a visible solve. Set
> `bootstrapWaf({ headless: false })` in [`demo/server/waf.mjs`](./demo/server/waf.mjs),
> solve the CAPTCHA once, and retry.

> The same real-vs-mock switch applies to the MCP server: `pnpm start` (real) vs
> `pnpm demo` (mock) in [`mcp/`](./mcp/), with the demo deps installed for real mode.
...
| Path | What it is |
| --- | --- |
| [`README.md`](./README.md) | This reference (protocol, auth, endpoint catalogue) |
| [`openapi.yaml`](./openapi.yaml) | OpenAPI 3.1 spec of the REST surface |
| [`persistent-poll.mjs`](./persistent-poll.mjs) | Minimal Node refresh-loop + chart poller |
| [`demo/`](./demo/) | Runnable React + Node proxy demo (WAF bootstrap, REST + WebSocket, `TR_DEMO=1` mock mode) |
| [`demo/web/api.html`](./demo/web/api.html) | **Scalar** API reference site (renders `openapi.yaml`) |
| [`mcp/`](./mcp/) | **MCP server** exposing the API as tools + resources to Claude — **self-contained** (own `lib/` + deps + bundled spec); copy it out to open-source it |

Run the demo (`cd demo && pnpm install && pnpm demo`) and open
`http://localhost:5175` — the live app, `/api.html` (Scalar), and `/docs.html`
(this guide) are all served from there.
